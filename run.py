                        ...
                        if episode_count > 0 and episode_count % 100 == 0:
                            try:
                                ...
                                if _wps and len(_wps) >= 4:
                                    ...
                                    _rl_eps = [{"steps": e.get("_steps", []), "completion_pct": e.get("lap_completion_pct", 0)} for e in bsts_feedback._all_summaries[-20:]]  # v1.1.7a FIX-RL-SCOPE
                                    _rl_score = score_race_line_compliance(_rl_eps, _rl)
                                    logger.info(f"[RaceLine] perp_v={_rl_score.get('avg_perp_v', 0):.4f}")  # v1.1.7a FIX-RL-SCOPE
