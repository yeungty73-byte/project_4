...
def run(hparams):
    ...
    ep_return = float('-inf')
    # v1.1.1: initialise _accel to 0.0 so it is always defined even before first rp block
    _accel = 0.0
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(num_steps):
            ...
            if rp:
                ...
                # v1.1.1 FIX: _accel hoisted here — was only defined in get_context block below,
                # causing UnboundLocalError when _jerk_now tried to read it first.
                _accel = (_speed - _step_speed_snap) / 0.1  # m/s² (dt≈0.1s) — canonical early definition
    ...
    #                     # reward += (anneal['reward_boost'] - 1.0) * 0.2 * _prog_r  # MUTED v25
            ...
            if hasattr(agent, 'get_context'):
                ...
                # v1.1.1: _accel already computed above in the rp block; recompute here with
                # the authoritative _step_speed_snap from this tick for the ante_buf record.
            ...
            # v1.1.1: update _step_speed_snap AFTER all reads (single authoritative write per step)
