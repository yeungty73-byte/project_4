...
if command_exists apptainer; then
    ...
    apptainer instance run \
        ...
        --bind configs:/configs \
            --bind /tmp/rollout_worker_patched.py:/opt/amazon/markov/rollout_worker.py \
        --overlay "$overlay"/:/. \
...
# check for Docker
    ...
    docker run --rm --detach \
        ...
        -v "$PWD"/custom_files:/opt/ml/code/custom_files:rw \
        -e WORLD_NAME="${WORLD_NAME:-reInvent2019_track}" \
        -p 8888:8888 \
